...
It's the same formula as the Coulomb's Law, but:
- The second charge is a test charge of magnitude 1 Coulombs. 
- We multiply by the unit vector that goes to this charge.

$$\Large \vec{E}=\frac{k_eq(1)}{r^2}\hat{r}=\frac{\vec{F}}{q}$$
> [!hint]
> We can write it as $\frac{\vec{F}}{q_1}$ simply because if you divide the Coulomb Force by one of the charges you get exactly the force applied on that charge from the other charge.
