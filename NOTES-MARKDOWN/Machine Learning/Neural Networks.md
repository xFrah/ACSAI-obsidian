...
Now we want to derive this(its [partial derivatives](../Calculus/old/Partial%20derivatives.md)) in order to obtain the [gradient](Gradient.md) of the loss function.
...
Since the chain rule computes the gradient of each intermediate variable and parameter ([partial derivatives](../Calculus/old/Partial%20derivatives.md)), it will also output the [gradients](Gradient.md) $\LARGE\frac{\partial\mathcal{L}}{\partial \mathbf{W}^1}$ and $\LARGE\frac{\partial\mathcal{L}}{\partial \mathbf{W}^2}$, which is exactly what we need.
...
> [!hint]
> Since the output is a [vector](../Linear%20Algebra/Vectors.md) composed of [partial derivatives](../Calculus/old/Partial%20derivatives.md), we can call it a [gradient](Gradient.md)!
> 
> $$\large\begin{bmatrix}
