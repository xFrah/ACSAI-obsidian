...
Basically, we define the running time of an algorithm by the length of its input (as stated above).
But this definition is encoding-dependent, since for example unary inputs (1111111111) are eponentially bigger than binary inputs. 
So in all explanations we assume we are using an encoding that is polynomially related to binary (hex, octa...)