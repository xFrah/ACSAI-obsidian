...
3. $q_0'$ is a state of $Q'$ that contains the starting state and the states reachable by the starting state using $\epsilon$ arrows.

![[Pasted image 20260110153019.png]]
...
> At every step in the com putation of $M$ on an input, it clearly enters a state that corresponds to the subset of states that $N$ could be in at that point. 
