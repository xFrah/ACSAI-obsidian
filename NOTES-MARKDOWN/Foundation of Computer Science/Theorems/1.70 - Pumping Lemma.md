...
Assume $A$ is a [[3. Regular Languages|regular language]]. We call the number of states of the automata that recognizes it $p$ (pumping length).
If $s$ is a string in $A$ of length at least $p$, then any splitting $s = xyz$ needs to satisfy the following conditions:
...
Let's consider the states of $M$ that $s=s_1s_2s_3\dots s_n$ goes through:

$$\Large r_{i+1}=\delta(r_i,s_i)\qquad 1\leq i\leq n$$

This means that the sequence of states that accepts $s$ has length $n+1$ (+1 for the starting state).
And we also know that **we selected a string that is longer than the automata** ($n\geq p$), this means that $n+1\geq p+1$.

**Pigeonhole principle**: We come to realize that if the string passes through more states than the automata has, it means one of its states must repeat. This **MUST ALWAYS OCCUR**.

![[Pasted image 20250813120256.png]]


We call the first occurrence of the repeating state in the accepting sequence $j$, and the second occurrence $l$.

**Now for the actual lemma:**
We should **always** be able to divide the string into 3 parts that satisfy the pigeonhole principle:

$$\Large \underbracket{x=s_0\dots s_{j-1}\qquad \underbracket{y=s_j\dots s_{l-1}}_{\text{loop}}}_{\text{within the automata length}}\qquad z=s_l\dots s_n$$

- Make sure that $j\neq l$, and so that $|y|>0$
- In order to use the pigeonhole principle, make sure to pick a split in which the loop happens before we could possibly exhaust the states of the automata. Written as $|xy|\leq p$.

> [!danger]
> When you pick a specific long string, you don't know exactly where its cycle will be, but you _know_ it must occur within the first $p$ characters, because the automata is there lol.

![[Pasted image 20260111112931.png]]

> [!danger] SUPER IMPORTANT TLDR
> When we pick a **string lengthier than the automata**, a **loop must exist** for it to accept the string.
> 
> But what if the $y$ we choose can't be looped in the automata? 
> What we need to prove, after selecting a string, is that **NO SPLIT CAN WORK**.
> If we prove that there isn't a split that can work with that string, the language is non-regular.

---

## FAQ

### Why can we just find a counterexample?

The pumping lemma says: 
**"IF a language is regular, THEN for ANY string in that language of sufficient length, the pumping conditions must hold."**
